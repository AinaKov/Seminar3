...
Например, можно пользоваться сервисом GitHub

![Логотип GitHub](GitHub-Logo.png)

Для этого необходимо создать аккаунт

### Создание нового удаленного репозитория

Разделим на два этапа

1. Создаем репозиторий на платформе GitHub
* Нажимаем "+" в правом верхнем углу
* Выбираем New repository
* Присваеваем Имя
* Выбираем дальнейшие действия:
    + ...or create a new repository on the command line - создать новый репозиторий через терминал и начать с ним работать
    + ...or push an existing repository from the command line - уже существующий репозиторий привязать к удаленному репозиторию
    + ...or import code from another repository - импортировать код из другого репозитория
2. Рассмотрим вариант привязки существующего к удаленному.
* Создаем локальный репозиторий
* Затем осуществляется связь между локальным репозиторием и удаленным. Некий обмен ключами, который в народе называют *"Подружить"*.
* Отправляем локальный репозиторий в удаленный репозиторий.

### Отправка изменений в удаленный репозиторий

Изменения, выполненные в локальном репозитории отправиляются в удаленный репозиторий с помощью команды:

    Git push

### Получение изменений

Обратное действие команде Git push это

    Git pull

оно позволяет выкачать актуальное состояние удаенного репозитория.

## Ответвление от чужого репозитория

На платформе Git Hub можно встретить множетсво репозиториев выполненных другими людьми. Если возникает необходимость внести изменения в чужой репозиторий, делается это через некий запрос

    Pull request

то есть мы как бы предлагаем автору ознакомиться с нашими предложениями.

### Порядок действий

1. Делаем **Fork** интересующего нас репозитория
2. Выполняем команду **Git clone** для создания нашей версии этого репозитория
3. Создаем ветку с предлагаемыми изменениями
4. Производим все изменения только в данной ветке
5. Отправляем эти изменения на свой аккаунт командой **Git push**
6. В окне GitHub появляется возможность отправить **Pull request**
(если не появилось, то нажимаем *Contribute*)
...
правая кнопка мыши по папке -> Открыть во встроенном терминале
